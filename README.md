...
- [류승환]
  ...
  - 전처리 기법 중 Merge를 Overlap 방식과 Multiply 방식을 비교했습니다.
  - Multiply방식의 Merge를 이용한 모델들간의 성능을 비교했습니다.
  - 모델관련 하이퍼 파라미터 및 전체적인 파이프라인을 논의했습니다.
- [이주영]
  - https://github.com/leekj3133
  - **팀장**
  - 전처리 기법 Invert와 VGG19모델을 이용하여 알츠하이머 MRI Image Data 예측 모델을 구축했습니다.
  - 모델관련 하이퍼 파라미터 및 전체적인 파이프라인을 논의했습니다.
...
****************************
## 4. CONCLUSION
- VGG19모델에서 전처리 기법인 Invert를 활용한 연구방안을 제언했습니다.
- 사용했던 모델 뿐 아니라 Multiply Merge의 특징을 더 부각시킬 수 있는 모델을 찾아 성능을 개선시키는 연구를 제언했습니다.
*********

## 5. comment & limitations
- 시간과 GPU가 부족해 원하는 많은 모델을 학습시키는데 한정적
- Train Result와 초기 입력한 Parameter가 저장되지 않아(파이프라인을 다시 실행할 경우) 추가 저장하지 않으면 유실될 가능성이 있습니다.
- Test Result를 DEEP:PHI 내에서만 결과를 볼 수 있어 불편했습니다.
- 데이터 출처에 대한 정확한 정보가 없기 때문에 이를 알아보기 위해 discussion을 찾아 보았으며, 이전에 각종 질문이 있었으나 정작 원작자 본인으로부터 답변을 받지 못했습니다.(신뢰성 부족)
- 출처가 정확한 데이터가 필요하였고, ADNI 사이트에서 신청하였지만 계속해서 승인대기중인 상태입니다.
